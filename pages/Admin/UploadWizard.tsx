...
export default function UploadWizard() {
  ...
  // Issue 4 & Issue 3 helper: Deterministic ID
  const [videoId] = useState(() => id || doc(collection(db, 'videos')).id);

  // Issue 1: Upload State
  ...
  // Issue 1 & Issue 4 Implementation
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Issue 1: Start upload immediately
      startUpload(file);
    }
  };
  ...
  const handleSave = async () => {
    if (!isFirebaseConfigured()) {
      alert("Action restricted: This action requires Firebase config.");
      return;
    }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          ...
          <button 
            onClick={handleSave}
            disabled={!isFirebaseConfigured() || isUploading || (!storagePath && !id)}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
        ...
        <div className="p-8">
            ...
            <div className="space-y-6">
              ...
              {!id && sourceType === 'direct_upload' && (
                <div 
                  ...
                  className={`p-10 border-2 border-dashed border-slate-200 rounded-2xl text-center transition-all group hover:border-sky-500 cursor-pointer ${isUploading ? 'bg-sky-50/20' : ''} ${!isFirebaseConfigured() ? 'bg-slate-50 opacity-60' : ''}`}
                >
                  ...
                  {isUploading ? (
                    <div className="max-w-xs mx-auto space-y-3">
                      ...
                      <div className="text-[10px] text-slate-400 font-mono">
                        {(uploadBytes.transferred / 1024 / 1024).toFixed(1)} MB / {(uploadBytes.total / 1024 / 1024).toFixed(1)} MB
                      </div>
                    </div>
                  ) : selectedLocalFile || storagePath ? (
                    <div className="space-y-2">
                      <h3 className="font-bold text-green-600">
                        {storagePath ? 'File Ready: ' : 'Selected: '}{selectedLocalFile?.name || 'Video File'}
                      </h3>
                      <p className="text-xs text-slate-400">Click to change video file</p>
                    </div>
                  ) : (
                    <>
                      <h3 className="font-bold text-slate-900">Choose Video File</h3>
                      <p className="text-sm text-slate-500 mt-1">MP4, MOV, MKV or WebM (Max 2GB). Will be automatically encrypted.</p>
                      <button 
                        type="button"
          ...
          {activeTab === 'availability' && (
            <div className="space-y-8">
              {/* Issue 2: Link Controls UI Implementation */}
